# -*- coding: utf-8 -*-
"""Copy_of_Assignment_2_for_DS_207_2025_(Intro_to_NLP)_Language_Modelling (1).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1cRfAU0r3uQNPAiMddjD8dZp0Mg-ZUCD0
...
STUDENT_SAP_NAME  = "Anshika Modi"
STUDENT_SR_NUMBER = "24095"
...
MAX_NAME_LENGTH = 18 # maximum length of names for generation
...
class NGramLanguageModel(object):
    def __init__(self, train_text,n=1):
        """
...
class UnigramModel(NGramLanguageModel):
    def __init__(self, train_text):
        ...
        self.counts = get_unigram_counts(train_text)
        self.train_text = train_text
        self.vocab = vocab.get_itos()
        self.vocab_size = len(self.vocab)
        self.total_chars = sum(self.counts.values())
    ...
    def get_name_log_probability(self, name):
        ...
        for char in name:
              prob = next_char_prob[char]
              name_log_probability+= np.log(prob) if char in next_char_prob and prob>0 else 0.0
        # END CODE
    ...
    def get_perplexity(self, text):
        ...
        total_len = sum(len(name) for name in text)
        log_prob_sum = sum(self.get_name_log_probability(name) for name in text)
        perplexity = np.exp(-log_prob_sum/total_len) if total_len>0 else float('-inf')
    ...
    def generate_names(self, k, n=MAX_NAME_LENGTH, prefix=None):
        ...
        keys = list(next_char_probs.keys())
        w8 = list(next_char_probs.values())
        if prefix:
          prefix = prefix[2:] if prefix[0] and prefix[1] == START else prefix
          prefix = prefix[:-1] if prefix[-1] == END else prefix

        for _ in range(k):
            name = prefix if prefix else []
            while len(name) < n:
                next_char = random.choices(keys, weights=w8)[0]
                if next_char == END:
                    break
                if next_char == START:
                    continue
    ...
    def get_most_likely_chars(self, sequence, k):
        ...
        next_char_probs = {char: prob for char,prob in self.get_next_char_probabilities().items() if char not in (START, END, UNK)}
        sorted_chars = sorted(next_char_probs.items(), key=lambda x: x[1], reverse=True)[:k]
        most_likely_chars= [char for char, _ in sorted_chars]
        # END CODE
    ...
    def get_position_specific_probabilities(self, max_position=5):
        ...
        position_distributions = [{key:0 for key in self.vocab if key not in (START, END, UNK)} for _ in range(max_position)]
        for name in self.train_text:
            for i in range(min(len(name), max_position+1)):
               if name[i] in position_distributions[i-1]:
                  position_distributions[i-1][name[i]] += 1

        for pos in position_distributions:
          total_count = sum(pos.values())
          for char in pos:
              pos[char] /= total_count if total_count > 0 else 0
...
## Please do not change anything in this code block.
position_distributions = unigram_model.get_position_specific_probabilities(max_position=7)
...
class SmoothedUnigramModel(UnigramModel):
    ...
    def get_next_char_probabilities(self):
        ...
        total = self.total_chars + self.vocab_size
        next_char_probabilities = {char: (self.counts[char]+1)/total for char in self.vocab}
        return next_char_probabilities
...
class BigramModel(NGramLanguageModel):
    def __init__(self, train_text):
        ...
        self.counts = get_bigram_counts(train_text)
        self.train_text = train_text
        self.vocab = vocab.get_itos()
        self.vocab_size = len(self.vocab)
        # END CODE
    ...
    def get_next_char_probabilities(self):
        ...
        for char1 in self.vocab:
            total_count = sum(self.counts[char1].values()) if char1 in self.counts else 0
            next_char_probabilities[char1] = {char: self.counts[char1][char] /total_count if total_count > 0 else 0 for char in self.vocab}
            if(total_count == 0):
    ...
    def get_name_log_probability(self, name):
        ...
        for i in range(1, len(name)):
            prob = next_char_prob[name[i - 1]][name[i]]
            name_log_probability += np.log(prob) if prob > 0 else 0.0
    ...
    def get_perplexity(self, text):
        ...
        name_log_prob=0
        for name in text:
          name_log_prob += self.get_name_log_probability(name)
          total_len += len(name)
        perplexity =  np.exp(-name_log_prob/total_len) if total_len>0 else float('-inf')
    ...
    def generate_names(self, k, n=MAX_NAME_LENGTH, prefix=None):
        ...
        for _ in range(k):
          name = []
          if prefix:
            name = prefix[2:] if prefix[0] and prefix[1]==START else prefix
          while len(name) < n:
            char = name[-1] if prefix else START
            if(char not in next_char_prob):
              break
            probs = next_char_prob[char]
            w8 = list(probs.values())
    ...
    def get_most_likely_chars(self, sequence, k):
        ...
        char = sequence[-1] if sequence else START
        prob = self.get_next_char_probabilities()[char]
        sorted_chars = sorted(prob.items(), key=lambda char: char[1], reverse=True)
        most_likely_chars = [c for c,_ in sorted_chars[:k]]
    ...
    def get_position_specific_probabilities(self, max_position=5):
        ...
        for name in self.train_text:
            name = [START] + name + [END]
            for i in range(min(len(name), max_position)):
                if name[i] not in position_distributions[i]:
                    position_distributions[i][name[i]] = defaultdict(int)
                position_distributions[i][name[i]][name[i+1]] += 1


        for pos in position_distributions:
          for prev, next_chars in pos.items():
             total_count = sum(next_chars.values())
             pos[prev]= {char: count / total_count for char, count in next_chars.items()}
...
BIGRAM_LAPLACE_K = 4 # value of k for add-k or Laplace smoothing in bigram models
# END CODE
...
class LaplaceSmoothedBigramModel(BigramModel):
    ...
    def get_next_char_probabilities(self):
        next_char_probabilities = {}
        for prev in self.vocab:
            total_count = sum(self.counts[prev].values()) if prev in self.counts else 0
            total = (self.k * self.vocab_size) + total_count
            next_char_probabilities[prev] = {char :(self.counts[prev][char]+self.k)/total for char in self.vocab}
        return next_char_probabilities
...
BIGRAM_LAMBDAS = (0.6 , 0.4) # lambdas for interpolation smoothing in bigram models
# END CODE
...
class InterpolationSmoothedBigramModel(BigramModel):
    ...
    def get_next_char_probabilities(self):
        ...
        for prev in self.vocab:
            total_count = sum(self.counts[prev].values()) if prev in self.counts else 0
            next_char_probabilities[prev] = {}
            for curr in self.vocab:
                  u_prob = (self.unigram_counts[curr])/self.total_unigram_count
                  b_prob = self.counts[prev][curr]/total_count if total_count > 0 else 0
                  next_char_probabilities[prev][curr] = (self.l1 * b_prob) + (self.l2 * u_prob)
            if(total_count == 0):
              next_char_probabilities[prev][curr] +=self.l1
        return next_char_probabilities
...
class TrigramModel(NGramLanguageModel):
    def __init__(self, train_text):
        ...
        self.t_counts = get_trigram_counts(train_text)
        self.b_counts = get_bigram_counts(train_text)
        self.u_counts = get_unigram_counts(train_text)
        self.l1 = TRIGRAM_LAMBDAS[0]
        self.l2 = TRIGRAM_LAMBDAS[1]
        self.l3 = TRIGRAM_LAMBDAS[2]
        self.vocab = vocab.get_itos()
        self.vocab_size = len(vocab)
        self.train_text = train_text
        self.total_unigram_count =  sum(self.u_counts.values())
    ...
    def get_next_char_probabilities(self):
        ...
        for prev_prev in self.vocab:
            for prev in self.vocab:
                if prev_prev == START and prev !=START:
                    continue
                prob_sum=0
                for curr in self.vocab:
                        bc = sum(self.b_counts[prev].values())  if prev in self.b_counts else 0
                        tc =  sum(self.t_counts[prev_prev][prev].values()) if (prev_prev in self.t_counts and prev in self.t_counts[prev_prev]) else 0
                        u_prob = (self.u_counts[curr])/(self.total_unigram_count) if curr in self.u_counts else 0
                        b_prob = (self.b_counts[prev][curr])/bc if bc > 0 and curr in self.b_counts[prev] else 0
                        t_prob = self.t_counts[prev_prev][prev][curr]/tc if tc > 0 and curr in self.t_counts[prev_prev][prev] else 0
                        final = (self.l1*t_prob) + (self.l2*b_prob) + (self.l3*u_prob)
                        next_char_probabilities[prev_prev][prev][curr] = final
                        prob_sum+=final

                for curr in next_char_probabilities[prev_prev][prev].keys():
                    next_char_probabilities[prev_prev][prev][curr] /= prob_sum
    ...
    def get_name_log_probability(self, name):
        ...
        next_char_prob = self.next_char_probabilities
        name_log_probability = 0.0
        for i in range(2, len(name)):
            prob = next_char_prob[name[i-2]][name[i - 1]][name[i]]
            name_log_probability += np.log(prob) if prob > 0 else 0.0
    ...
    def get_perplexity(self, text):
        ...
        perplexity = np.exp(-name_log_prob/total_len)
        # END CODE
        return perplexity
    ...
    def generate_names(self, k, n=MAX_NAME_LENGTH, prefix=None):
        ...
        for _ in range(k):
          name = prefix if prefix else []
          while(len(name)<=n+1):
               prev = name[-1] if name else START
               prev_prev = name[-2] if len(name)>=2 else START
               if(prev == END):
                 break
               probs = next_char_prob[prev_prev][prev]
               if not probs:
                 break
               next_char = random.choices(list(probs.keys()), weights=list(probs.values()))[0]
               if(next_char == END):
                 break
               if next_char == START:
                 continue
    ...
    def get_most_likely_chars(self, sequence, k):
        ...
        if len(sequence)>=2:
            prev_prev, prev = sequence[-2], sequence[-1]
        elif len(sequence) ==1:
            prev_prev, prev = START,sequence[-1]
        else:
            prev_prev, prev = START, START
        prob = self.get_next_char_probabilities()[prev_prev][prev]
        sorted_chars = sorted(prob.items(), key=lambda char: char[1], reverse=True)[:k]
        most_likely_chars = [ c for c,_ in sorted_chars]
    ...
    def get_position_specific_probabilities(self, max_position=5):
        ...
        position_distributions = [{} for _ in range(max_position)]
        for name in self.train_text:
            name = [START, START] + name + [END]
            for i in range(2,min(len(name), max_position+2)):
                prev = (name[i-2],name[i-1])
                curr = name[i]
                if prev not in position_distributions[i-2]:
                    position_distributions[i-2][prev] = defaultdict(int)
                position_distributions[i-2][prev][curr] += 1
        for pos in position_distributions:
            for prev, next_chars in pos.items():
                total_count = sum(next_chars.values())
                pos[prev] = {char: count / total_count for char, count in next_chars.items()}
...
class FNN_LM(nn.Module):
    ...
    def forward(self, chars):
        ...
        x = self.emb_layer(chars)
        x = x.view(-1,(self.ngram-1)*self.emb_size)
        fc1 = self.fc1(x)
        logits = self.fc2(self.tanh(fc1))
        # END CODE
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
           ...
           for batch in self.train_dataloader:
               ip, lab = batch[0], batch[1]
               self.optimizer.zero_grad()
               op = self.model(ip)
               loss = self.criterion(op, lab)
               loss.backward()
               self.optimizer.step()
           ...
           # Print epoch loss
           print(f"Epoch: {epoch + 1}/{self.epochs}, Train Loss={avg_train_loss:.5f}, Val Loss={avg_val_loss:.5f}")
...
class FNN_Inference:
    ...
    def get_next_char_probabilities(self, prefix=None):
        ...
        with torch.no_grad():
            next_char_probabilities = {}
            seq = [START]*(self.ngram-1)
            ip_tensor = torch.tensor([[stoi[char] for char in seq]], dtype=torch.long)
            if torch.cuda.is_available():
              ip_tensor = ip_tensor.cuda()
            probabilities = torch.softmax(self.model(ip_tensor), dim =1)[0]
            next_char_probabilities= {itos[i]: prob.item() for i, prob in enumerate(probabilities)}
    ...
    def generate_names(self, k, n, prefix=None):
        ...
        with torch.no_grad():
            for _ in range(k):
                curr_prefix = [START] * (self.ngram - 1) if prefix is None else [START] * (self.ngram - 1) + prefix
                prefix = [] if prefix is None else prefix
                name = []
                while(len(name)<n):
                    ip_idx = [stoi[char] for char in curr_prefix[-self.ngram+1:]]
                    ip_tensor = torch.tensor([ip_idx], dtype=torch.long)
                    if torch.cuda.is_available():
                        ip_tensor = ip_tensor.cuda()

                    probabilities = torch.softmax(self.model(ip_tensor)[-1], dim=0)
                    next_idx = random.choices(range(len(probabilities)) , probabilities , k = 1)[0]
                    next_char = itos[next_idx]
                    if next_char == END:
                        break
                    name.append(next_char)
                    curr_prefix = curr_prefix + [next_char]

                generated_names.append(''.join(prefix[2:]+name))
    ...
    def get_perplexity(self, text):
        ...
        with torch.no_grad():
            for name in text:
                log_prob_sum = 0.0
                seq = [START] * (self.ngram - 2) + name + [END]
                for idx in range(self.ngram - 1, len(name) - 1):
                    inp_seq = [stoi[char] for char in seq[idx - self.ngram + 1:idx]]
                    inp_tensor = torch.tensor([inp_seq], dtype=torch.long)
                    if torch.cuda.is_available():
                        inp_tensor = inp_tensor.cuda()
                    probabilities = torch.softmax(self.model(inp_tensor)[-1].cpu(), dim=0)

                    next_char_prob = probabilities[stoi[name[idx + 1]]].item()
                    log_prob_sum += np.log(next_char_prob)

                total += np.exp(-log_prob_sum / len(name))

        perplexity = total / num
    ...
    def get_most_likely_chars(self, sequence, k):
        ...
        with torch.no_grad():
            ...
            ip_idx = [[stoi[char] for char in prefix]]
            ip_tensor = torch.tensor([ip_idx], dtype=torch.long)
            if torch.cuda.is_available():
                ip_tensor = ip_tensor.cuda()
            probabilities = torch.softmax(self.model(ip_tensor), dim=1)
            sorted_indices = torch.argsort(probabilities[0], descending=True)
            most_likely_chars = [itos[idx] for idx in sorted_indices[:k]]
...
class RNN_LM(nn.Module):
    ...
    def forward(self,data):
        ...
        rnn_op , hidden_state = self.rnn(embedding)
        logits = self.linear(rnn_op)
        return logits
    # END CODE
...
class RNNTrainer:
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            train_loss = []
            for batch in self.train_dataloader:
                ip, lab = batch[0].to(device), batch[1].to(device)
                self.optimizer.zero_grad()
                op = self.model(ip).view(-1, vocab_size)
                lab = lab.view(-1)
                loss = self.criterion(op, lab)
                loss.backward()
                self.optimizer.step()
                train_loss.append(loss.item())
            self.loss['train'].append(np.mean(train_loss))
            ...
            print(f"Epoch: {epoch + 1}/{self.epochs}, Train Loss={self.loss['train'][-1]:.6f}, Val Loss={self.loss['val'][-1]:.6f}")
...
class RNN_Inference:
    ...
    def get_next_char_probabilities(self):
        ...
        with torch.no_grad():
            next_char_probabilities = {}
            st = [START]
            ip = torch.tensor([[stoi[char] for char in st]], dtype= torch.long)
            if torch.cuda.is_available():
              ip = ip.cuda()
            probs = torch.softmax(self.model(ip)[-1], dim =1).cpu().detach().numpy()
            for i in range(len(probs[-1])):
              next_char_probabilities[itos[i]]= probs[-1][i]
    ...
    def generate_names(self, k, n, prefix=None):
        ...
        with torch.no_grad():
            for _ in range(k):
                curr_prefix = [START] if prefix is None else [START] + prefix
                prefix = [] if prefix is None else prefix
                name = []
                while len(name)<n:
                    ip_tensor = torch.tensor([self.vocab(curr_prefix)], dtype=torch.long)
                    if torch.cuda.is_available():
                        ip_tensor = ip_tensor.cuda()
                    probs = torch.softmax(self.model(ip_tensor)[-1][-1], dim=0)
                    next_idx = random.choices(range(len(probs)) , probs , k = 1)[0]
                    next_char = itos[next_idx]
    ...
    def get_perplexity(self, text):
        ...
        with torch.no_grad():
            for name in text:
              prob = 1
              for i in range(1 , len(name)):
                ip = torch.tensor([[stoi[char] for char in name[:i]]], dtype= torch.long)
                if torch.cuda.is_available():
                  ip = ip.cuda()
                op = torch.softmax(self.model(ip)[-1][-1], dim =0).cpu()
                prob *=op[stoi[name[i]]]
              perplexity += (1/prob) ** (1/len(name))
        perplexity /= len(text)
        #END CODE
        return perplexity
    ...
    def get_most_likely_chars(self, sequence, k):
        ...
        self.model.eval()
        itos = self.vocab.get_itos()
        prefix = sequence if sequence else [START]
        with torch.no_grad():
            ip_tensor = torch.tensor([self.vocab(prefix)], dtype=torch.long)
            if torch.cuda.is_available():
                ip_tensor = ip_tensor.cuda()

            probs = torch.softmax(self.model(ip_tensor)[-1][-1], dim=0)
            sort = torch.argsort(probs, descending=True)
            most_likely_chars = [itos[idx] for idx in sort[:k]]
